$(function () {
    $('#tblShopTireAll').dataTable();
    //$('#tblShopTire_length').hide();
    //$('#tblShopTire_filter').hide();
    ShopTireAllDataTable('/ShopTireAll/ShopTireAllList/');
    //choosen();
...
function ShopTireAllDataTable(url) {

    getShopTireAllData(url);
    //$('#tblShopTire_length').hide();
    //$('#tblShopTire_filter').hide();
}
...
function getShopTireAllData(url) {
    var stores = "";
    var title = "";
    var imageName = "";
    var viewDevice = "";
    ...
    var languageId = "";
    //debugger;
    var table = $('#tblShopTireAll').DataTable({
        "ajax": {
            "url": url,
            "type": "Post",
            "data": { title: title, imageName: imageName, viewDevice: viewDevice, languageId: languageId},
            "datatype": "json",
        },
        autoWidth: false,
        "columns": [
            ...
            { "data": "imageName", "name": "imageName", "width": "20%" },
            { "data": "viewDevice", "name": "viewDevice", "width": "20%" },
            /*{ "data": "imageStatus", "name": "imageStatus", "width": "20%" },*/
            { "data": "languageId", "name": "languageId", "width": "20%" },
            { "data": "Action", "name": "non", "width": "20%" }
        ],
        "columnDefs": [{
        ...
        },
        {
            ...
            "render": function (data, type, full, meta) {
                var viewUrl = "'/ShopTireAll/Details/" + full.guid + "'";
                var editUrl = "'/ShopTireAll/Edit/" + full.guid + "'";
                ...
                actionUrl = actionUrl + '<button class="mb-0 mr-2 btn-icon btn-icon-only btn btn-primary" onclick="editShopTireAllData(' + editUrl + ')" title = "Edit ShopTire Banner"><i class="pe-7s-pen btn-icon-wrapper"></i></button>'
                    + '<button class="mb-0 mr-2 btn-icon btn-icon-only btn btn-warning" onclick="viewShopTireAllData(' + viewUrl + ')" title = "View ShopTire Banner"><i class="pe-7s-look btn-icon-wrapper"></i></button>'
...
function viewShopTireAllData(url) {
    var url = url;
...
function editShopTireAllData(url) {
    var url = url;
    var arrUrl = url.split("/");
    var guid = arrUrl[3];
    $.ajax({
...
function deleteShopTireAllData(url) {
    ...
    var arrUrl = url.split("/");
    var guid = arrUrl[3];
    if (confirm("Do you want to delete the ShopTire-All Banner?")) {
        this.click;
        //alert("Ok");
        $.ajax({
            type: "Get",
            ...
            success: function (data) {
                if (data != null) {
                    ...
                    if (data.data.isSuccessfull === true && data.data.formMode === "Delete") {
                        getShopTireData('/ShopTireAll/ShopTireAllList');
        ...
        success: function (data) {
            if (data != null) {
                if (data.data.isSuccessfull === true) {
                    toastr.success(data.data.message);
                    getShopTireAllData('/ShopTireAll/ShopTireAllList');
                    //$('#modalCMS').modal('hide');
                }
                else {
                    toastr.error(data.data.message);
...
function closeModal() {
    var url = '/ShopTireAll/ShopTireAllList';
    $('#modalCMS').modal('hide');
    getShopTireAllData(url);
}
...
function validateFileResolution(file, event) {
    ...
    reader.onload = function (e) {
        ...
        image.onload = function () {
            ...
            var url = '/ShopTireAll/ValidateFileResolution';
            $.ajax({
                type: 'POST',
                url: url,
                async: false,
